...
export class ExamResultComponent implements OnInit {
  ...
  ngOnInit() {
    this.route.paramMap.subscribe((params) => {
      ...
      this.answers = this.allAnswers.filter(
  ...
  checkTestReult() {
    for (let index = 0; index < this.questions.length; index++) {
      const answers = this.allAnswers.filter(
        (a) => a.QuestionId == this.questions[index].ID
      );
      debugger;
      const correctAnswersId = answers
        .filter((a) => a.CorrectAnswer)
        .map((a) => a.ID);
      const selectedAnswersId = answers
        .filter((a) => a.Selected)
        .map((a) => a.ID);
      const testResult = {
    ...
    for (let i = 0; i < this.testReuslts.length; i++) {
      for (let j = 0; j < this.testReuslts[i].correctAnswersId.length; j++) {
        if (
          this.testReuslts[i].correctAnswersId[j] !=
          this.testReuslts[i].selectedAnswersId[j]
        ) {
          this.testReuslts[i].userAnsweredCorrectly = false;
        }
      }
    }
    this.numberOfRightAnswers = this.testReuslts.filter(
      (tr) => tr.userAnsweredCorrectly == true
    ).length;
    this.calculateGrade();
  }
  ...
  calculateGrade() {
    ...
    this.examService.generateReport(report).subscribe(
      (report) => {},
      (err) => console.log(err)
    );
  }
